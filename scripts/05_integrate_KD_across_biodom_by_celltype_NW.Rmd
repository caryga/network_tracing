...
doms <- doms %>% mutate(bdct = paste0(bd,' - ', celltype))
```
...
kd <- map_dfr(
  ...
  kd.res <- doms %>% filter(bdct == .x) %>% 
    pull(nw.dir) %>% .[[1]] %>% filter(name == 'kda') %>% 
    pull(id) %>% read_kd_remote(.) %>% try()
    
  if(class(kd.res)[1]=='try-error'){ return(tibble(nw = .x)) }
...
top.kd <- kd %>%
  select(network = nw, n_nodes, kd_biodom = Biodomain,
         ...
         color ) %>%
  filter(ef1_fdr > -log10(0.05), delta_fdr > 0, top_kd == 1, pct_term > 25)
...
tmp <- kd %>% 
  ...
  # ) %>%
  filter(grepl('Synapse|Immune_Response|Lipid_Metabolism', nw)) %>% 
  group_by(nw,Biodomain) %>% 
  mutate(n_kd = length(unique(paste0(NODE,':',MODULE)))) %>% 
  ungroup() %>% 
  mutate(Biodomain = fct_reorder(Biodomain, n_kd),
         ct = str_split_fixed(nw, ' - ',2)[,2],
         bd_nw = str_split_fixed(nw, ' - ',2)[,1],
         nw = paste0(nw, '\n', n_nodes, ' nodes'),
...
ggplot(tmp, aes(delta_fdr, Biodomain))+
  ...
  geom_jitter(data = subset(tmp, (top == 1 & ef1_fdr > -log10(0.05) & delta_fdr > 0 & pct_term > 25)==F ), 
              color = 'grey90', alpha = .1, size = .5, shape = 21)+
  geom_jitter(data = subset(tmp, (top == 1 & ef1_fdr > -log10(0.05) & delta_fdr > 0 & pct_term > 25)==T ), 
              alpha = .5, shape = 21, color = 'grey30', aes(fill = color, size = ef1_fdr))+
  geom_violin(data = subset(tmp, (top == 1 & ef1_fdr > -log10(0.05) & delta_fdr > 0 & pct_term > 25)==T ),
  ...
  #   max.overlaps = 100)+
  facet_grid(bd_nw~ct, scale = 'free_x')+ 
...
tmp <- top.kd %>% 
  ...
  mutate(kd_biodom = fct_reorder(kd_biodom, n_kd),
         ct_nw = str_split_fixed(network, ' - ',2)[,2],
         bd_nw = str_split_fixed(network, ' - ',2)[,1],
...
ggplot(tmp, aes(ef1_fdr, kd_biodom))+
  ...
  labs(y = '', x = '\u0394 FDR, ef1 - ef0')+
  facet_grid(ct_nw~bd_nw,  scale = 'free_x')+ # 
  scale_fill_identity()+scale_color_identity()
```

```{r fig.width=8, fig.height=6}
tmp <- top.kd %>% 
  filter(grepl('Immune_Response|Synapse|Lipid_Metabolism', network)) %>% 
  group_by(network,kd_biodom) %>% 
  mutate(n_kd = length(unique(paste0(NODE,':',MODULE)))) %>% 
  ungroup() %>% 
  mutate(kd_biodom = fct_reorder(kd_biodom, n_kd),
...
ggplot(tmp, aes(ef1_fdr, kd_biodom))+
  geom_jitter(alpha = .5, shape = 21, color = 'grey30', aes(fill = color))+
  geom_violin(aes(color = color), scale = 'width', alpha=.3)+
  labs(y = '', x = '\u0394 FDR, ef1 - ef0')+
  facet_grid(bd_nw~ct_nw,  scale = 'free_x')+ # 
  scale_fill_identity()+scale_color_identity()
```
...
for(j in 1:length(dom)){
  ...
  vert_neigh <- neighbors(snw, vert_num[!is.na(vert_num)])

  g3 <- induced_subgraph(snw, c(vert_num[!is.na(vert_num)], vert_neigh))
...
```{r fig.width=8, fig.height=6}
ggplot(tmp, aes(sum_delta_rank))+ 
  geom_density(alpha = .3, aes(fill = AgoraNom))+
  scale_x_log10()+
  scale_fill_manual(values=c('grey80','blue'))
```
...
```{r fig.width=8, fig.height=6}
npath %>% 
  mutate(kd = if_else(gene %in% tmp$NODE, T, F)) %>% 
  ggplot(aes(NeuropathScore))+ 
  geom_density(alpha = .3, aes(fill = kd))+
  scale_x_log10()+
  scale_fill_manual(values=c('grey80','blue'))
```

```{r fig.width=8, fig.height=6}
left_join(tmp, npath %>% select(gene, NeuropathScore), by = c('NODE'='gene')) %>% 
  arrange(desc(NeuropathScore)) %>% 
  ggplot(aes(sum_fdr_rank, sum_delta_rank))+ 
  geom_abline(intercept = 0, slope = 1, lty = 2, lwd = .5)+
  geom_point(alpha = .3, shape = 21, color = 'grey30', aes(size = n_nw, fill = NeuropathScore))+
  viridis::scale_fill_viridis()+
  ggrepel::geom_label_repel(
    data = subset(tmp, n_nw >= 3), aes(label = NODE)
    , size = 2, min.segment.length = 0, max.overlaps = 20)
...
ggplot(tmp, aes(sum_fdr_rank, sum_delta_rank))+ 
  ...
  scale_fill_identity()+ 
  ggrepel::geom_label_repel(
    data = subset(tmp, n_nw >= 6 & MODULE %in% enr.bd$pathway[which(enr.bd$padj < 0.05)] ),
...
for(i in 1:length(bd)) {
  ...
  # } else {
    p <- ggplot(
      ...
      scale_fill_identity()+ 
      ggrepel::geom_label_repel(
        data = subset(tmp, n_nw >= 5 & kd_biodom == bd[i]
