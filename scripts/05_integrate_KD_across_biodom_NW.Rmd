...
doms <- synGetChildren('syn51739831')$asList() %>%
  ...
  mutate(
    ...
    kd.dir = synGetChildren(  parent = nw.dir$id[grepl('kda',nw.dir$name)]  )$asList() %>% tibble(f = .) %>% 
      unnest_wider(f) %>% list()
    )
```
...
ef0 = kd %>% filter(nw == 'Mitochondrial_Metabolism') %>% 
  pull(nw.dir) %>% .[[1]] %>% filter(name == 'kda') %>% pull(id) %>% 
  read_syn_dir(.) %>% filter(grepl('edgeFactor_0.results', name)) %>% pull(id) %>% 
  synGet(.) %>% .$path %>% read_tsv()

ef0_top = doms %>% filter(bd == 'Mitochondrial_Metabolism') %>% 
  pull(nw.dir) %>% .[[1]] %>% filter(name == 'kda') %>% pull(id) %>% 
  read_syn_dir(.) %>% filter(grepl('edgeFactor_0.tophits', name)) %>% pull(id) %>% 
  synGet(.) %>% .$path %>% read_tsv()

ef1 = doms %>% filter(bd == 'Mitochondrial_Metabolism') %>% 
  pull(nw.dir) %>% .[[1]] %>% filter(name == 'kda') %>% pull(id) %>% 
  read_syn_dir(.) %>% filter(grepl('edgeFactor_1.results', name)) %>% pull(id) %>% 
  synGet(.) %>% .$path %>% read_tsv()

ef1_top = doms %>% filter(bd == 'Mitochondrial_Metabolism') %>% 
  pull(nw.dir) %>% .[[1]] %>% filter(name == 'kda') %>% pull(id) %>% 
...
cowplot::plot_grid(p1,p2, nrow = 1, rel_widths = c(.8,1))
```
```{r fig.width=6, fig.height=5}
ggplot(data = subset(top.kd, network == 'Mitochondrial_Metabolism'), 
       aes(FDR_ef1, delta_fdr))+
  geom_hline(yintercept = 0, lwd = 0.3)+
  geom_abline(intercept = 0, slope = 1, lty =2, lwd = 0.3)+
  geom_point(size = 3, alpha = .5, color = 'grey30', 
  ...
  ggrepel::geom_label_repel(
    data = subset(top.kd, network == 'Mitochondrial_Metabolism' & !duplicated(NODE)),
...
tmp <- kd %>% 
  filter(nw == 'Mitochondrial_Metabolism') %>% 
  group_by(Biodomain) %>% 
...
ggplot(tmp, aes(delta_fdr, Biodomain))+
  ...
  scale_size_continuous('-log10(FDR), weighted',  range = c(.5,3), breaks = c(20, 200))+
  scale_color_identity()+scale_fill_identity()
```

```{r}
snw <- doms %>% filter(bd == 'Mitochondrial_Metabolism') %>% 
...
```{r}
top.kd %>%
  filter( network == 'Mitochondrial_Metabolism', 
  ...
  summarise(
    ...
    max_ef1_fdr = max(FDR_ef1)) %>% 
  arrange(desc(n_term), desc(max_ef1_fdr)) %>%
  DT::datatable() %>% 
  DT::formatSignif(columns = c('max_ef1_fdr', 'max_delta'))
```
...
top.kd %>%
  ...
  summarise(
    ...
    max_ef1_fdr = max(FDR_ef1)) %>% 
  arrange(desc(n_term), desc(max_ef1_fdr)) %>%
  ggplot(aes(n_term, max_delta))+
  geom_point()+
  ggrepel::geom_label_repel(aes(label = NODE), 
...
top.kd %>%
  ...
  select(NODE, MODULE, FDR_ef1, delta_fdr, top_kd) %>% 
  DT::datatable() %>% 
  DT::formatSignif(columns = c('FDR_ef1', 'delta_fdr'))
```
...
ggraph(g3, layout = 'graphopt') +
    ...
    geom_node_point( shape = 21, color = "grey50", alpha = 1,
                     aes(size = TargetRiskScore, fill = Pro_EffectScore, 
                         filter= .N()$name != kdn)  )+
    geom_node_point( shape = 23, color = "grey50", alpha = 1,
                     aes(size = TargetRiskScore, fill = Pro_EffectScore, 
                         filter = .N()$name == kdn) )+
    geom_node_text(aes(label = name), size = 2) +
    scale_size_continuous( range = c(3, 10), limits = c(0, 5), trans = 'exp')+
    scale_fill_gradient2(
...
ggplot(tmp, aes(delta_fdr, Biodomain))+
  geom_vline(xintercept = 0, lwd = 0.3)+
  geom_jitter(data = subset(tmp, (top == 1 & FDR_ef1 > -log10(0.05) & delta_fdr > 0 & pct_term > 50)==F ), 
              color = 'grey90', alpha = .1, size = .5, shape = 21)+
  geom_jitter(data = subset(tmp, (top == 1 & FDR_ef1 > -log10(0.05) & delta_fdr > 0 & pct_term > 50)==T ), 
              alpha = .5, shape = 21, color = 'grey30', aes(fill = color, size = FDR_ef1))+
  geom_violin(data = subset(tmp, (top == 1 & FDR_ef1 > -log10(0.05) & delta_fdr > 0 & pct_term > 50)==T ),
              aes(color = color), scale = 'width', alpha=.3, trim = T)+
...
tmp <- top.kd %>% 
  ...
  mutate(kd_biodom = fct_reorder(kd_biodom, n_kd),
         network = paste0(str_replace_all(network,'_','\n'), '\n\n', n_nodes, ' nodes'),
         network = fct_reorder(network, desc(n_nodes) )) %>% 
  arrange(kd_biodom)

ggplot(tmp, aes(FDR_ef1, kd_biodom))+
  geom_jitter(alpha = .5, shape = 21, color = 'grey30', aes(fill = color, size = delta_fdr))+
  geom_violin(aes(color = color), scale = 'width', alpha=.3)+
  labs(y = '', x = 'weighted KD -log10(FDR)', size = '\u0394 FDR\nweighted vs unweighted')+
  facet_wrap(~network, nrow = 2, scale = 'free_x')+ # 
...
## KD for each biodom across graphs {.tabset}  
...
```{r echo=FALSE,warning=FALSE,message=FALSE,results='asis', fig.width=7, fig.height=8}
dom = doms$bd %>% unique %>% sort
plot.new();dev.off()
for(i in 1:length(dom)) {
  ...
  # list of key driver
  kd.hits <- top.kd %>% filter(network == dom[i]) %>% pull(NODE) %>% unique()
      ...
      cbrain_lm_ef1 <- tmp %>%
        ...
        mutate(
          lm = if_else(
            ...
            lm(gs~FDR_ef1+node_degree) %>% broom::tidy() %>% filter(term == 'FDR_ef1') %>%
              mutate(r.squared = lm(gs~FDR_ef1+node_degree) %>% broom::glance() %>% pull(r.squared),
                     nobs = lm(gs~FDR_ef1+node_degree) %>% broom::glance() %>% pull(nobs)) %>% list()
          , )
            ) %>%
        unnest(lm)  %>%
...
ggplot(tmp, aes(sum_fdr_rank, sum_delta_rank))+ 
  ...
  scale_fill_identity()+ 
  ggrepel::geom_label_repel(
    data = subset(tmp, n_nw >= 4 & MODULE %in% enr.bd$pathway[which(enr.bd$padj < 0.05)] ),
...
```{r echo=FALSE,warning=FALSE,message=FALSE,results='asis', fig.width=5, fig.height=3}
bd = tmp$kd_biodom %>% unique %>% sort
plot.new();dev.off()
for(i in 1:length(bd)) {
  cat("  \n####",  bd[i], "  \n")
  if( (tmp %>% filter(kd_biodom == bd[i], n_nw  >= 2, MODULE %in% enr.bd$pathway[enr.bd$padj < 0.05]) %>% nrow())==0 ){
    ...
    p <- ggplot(
      ...
      ggrepel::geom_label_repel(
        data = subset(tmp, 
                      (n_nw >= 2 & kd_biodom == bd[i] & MODULE %in% enr.bd$pathway[enr.bd$padj < 0.05]) 
